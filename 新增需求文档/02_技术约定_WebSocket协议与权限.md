...
| `webrtc.offer` | `{toUserId, sdp}` | **新增** | 老师发给指定学生（M1） |
| `webrtc.answer` | `{toUserId, sdp}` | **新增** | 学生回信令（M1） |
| `webrtc.ice` | `{toUserId, candidate}` | **新增** | 双向 ICE 候选（M1） |
| `webrtc.ready` | `{}` | **新增** | 学生表示"可以接流了"（M1） |

> ### ⚠️ `webrtc.offer` 是双向的（初版遗漏，已补）
>
> 协商由**老师**发起，所以 `webrtc.offer` 既是服务端 → 学生的事件，**也是老师 → 服务端的上行消息**。
> 本表初版只把它列在下行，漏了上行那一半。
>
> **漏掉的症状极具迷惑性**：老师点「开始共享」一切正常（接口 200、状态已登记、学生也收到
> `stream.started`），**但学生端永远黑屏**——因为老师的 offer 在服务端被当成未知消息类型丢掉了，
> 学生从来没收到过 offer，协商根本没开始。
>
> 已实测确认并修复（见 P1 验证记录）。
