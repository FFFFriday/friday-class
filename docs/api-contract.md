...
```
- 响应 `data`：`null`
- **说明**：改密码会让 `token_version` +1，**此前签发的所有 JWT 立即失效**，
  必须重新登录（前端就是这么做的：改完主动登出并跳登录页）。

### 1.5 修改个人资料
- **PUT** `/api/auth/profile`（需登录）
- 请求体：
```json
{ "nickname": "张三" }
```
...
```
- **说明**：
  - **只能改昵称**。用户名 `username` 是登录凭据（JWT 主体 + 唯一索引的一列），
    改它等同于换账号，刻意不开放；要换用户名只能由管理员重建账号。
  - 昵称**允许重复** —— 它只是显示名，全库真正需要唯一的只有 `username`。
  - 校验：非空、最长 50 字。传空或超限返回 `code:400`
    （走统一信封，HTTP 状态码仍是 200）。
  - 与 1.4 的关键差别：**改昵称不会使当前 JWT 失效**（`token_version` 保持不变），
    所以改完名字不需要重新登录。
