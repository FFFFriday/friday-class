...
### 6.13 新增需求 P2：AI 会话与上下文 / 学生端知识可见（2026-09-21）

> 来源：`新增需求文档/` 的 M4（学生 AI 会话与上下文）、M3（学生端知识可见）。

#### 新增 REST 接口

| 方法 | 路径 | 鉴权 | 说明 |
|---|---|---|---|
| GET | `/api/ai/conversations` | 需登录 | 我的会话列表，按最后活跃倒序，带 `messageCount` |
| POST | `/api/ai/conversations` | 需登录 | 新建。`{coursewareId?, sessionId?, title?}` **三字段全可空** |
| GET | `/api/ai/conversations/{id}` | 需登录 | 会话头 + 全部消息（不分页，单会话上限 200 条） |
| PUT | `/api/ai/conversations/{id}` | 需登录 | 重命名 `{title}` |
| DELETE | `/api/ai/conversations/{id}` | 需登录 | 软删除。**问答记录保留**（课堂记录要用） |

**权限靠归属校验，不靠角色**：这组接口不加 `hasRole`——教师也可以有自己的 AI 会话。
真正的边界是 `requireOwned(studentId, id)`：不匹配返回 **404 而不是 403**，
返回 403 等于告诉对方「这个 ID 确实存在」，反而泄露信息。

#### `POST /api/qa/ask` 的改造（最小改动原则）

请求体新增**可选**的 `conversationId`，并且 `sessionId` / `pageId` **改为可空**：

```jsonc
{ "sessionId": 13, "pageId": 88, "conversationId": 42,   // 三者都可空
  "question": "...", "clientRequestId": "uuid" }
```

| 传了 conversationId | 用它（校验归属） |
|---|---|
| 没传 | 自动落到该学生在该课件下的「默认会话」，找不到就**自动新建** |

**没传也能跑**是刻意的：老前端不改也继续可用，新 UI 才传。

**三者都可空 = 课后提问**：学生不在任何课堂上时，`sessionId` 与 `pageId` 都是 null。
⚠️ 这依赖数据库把 `qa_record` 的 **`session_id` 与 `page_id` 双双改成可空**——
初版只放开了 `page_id`，课后提问直接 500（`Column 'session_id' cannot be null`）。

响应新增 `conversationId` 与 `turn`（本会话第几轮，从 1 开始）。老记录两者均为 null。

#### ⚠️ 「省钱两件套」已删除（2026-09-21）

`QaService` 原先有两处优化，**现已全部移除**：

| 移除项 | 原作用 | 移除理由 |
|---|---|---|
| 答案复用缓存（10 分钟 TTL） | 相同问题复用答案 | 按 Friday 指示不省钱；**且会跨会话串台** |
| 并发合流（single-flight） | 并发同问只调一次模型 | 同上 |

**「串台」到底指什么**：缓存的键是「课堂｜页｜问题文字」，**它根本不认识「会话」是什么**。
A 会话问「什么是 TCP」得到答案 X（按 A 的上下文生成），10 分钟内 B 会话问同一句就拿走 X——
B 看到的是别人上下文里的答案。

**保留**的是 `clientRequestId` + `uk_qa_client_req` 幂等：它不省模型钱，
是防「手抖双击重复扣费」，无害且必要。

#### 上下文组装

取该会话最近 `app.ai.context-turns`（默认 **6**）轮问答 → 反转成正序 → 拼成
「学生：…／助教：…」文本 → 与知识点、预置提问、本次问题一起送进提示词。

- 历史**同样走 `sanitize()` 包裹**并剥离定界符：历史里的学生提问是自由输入，
  上一轮塞一句「忽略之前的要求」，这一轮会以更高可信度重新进入提示词
  （模型更容易把「自己说过的话」当可信内容）。
- 每条按 `MAX_HISTORY_QUESTION_CHARS`(200) / `MAX_HISTORY_ANSWER_CHARS`(400) 截断。
- **不做**摘要压缩、向量检索、跨会话引用。

#### 两套 System Prompt（容易漏）

| 场景 | 用哪套 |
|---|---|
| 有本页知识点/预置提问 | `qaSystemPrompt()` —— 严格版：「只依据下面提供的参考资料回答」 |
| 课后自由问答（无资料） | `qaFreeSystemPrompt()` —— 通用版 |

**不能只有严格版**：课后提问时参考资料是空的，学生会问什么都得到
「这部分课件没有提到」——而「课后也能用 AI」是明确需求，那样等于功能不存在。
通用版仍然**不允许编造课件内容**，只是允许讲通用知识。

#### M3 学生端知识可见：其实是「拆掉一道门」

后端**一行都没改**。`GET /api/courseware/{id}/prompt-pack` 早就允许学生调用，
`usePromptPack` 也一直在拉——只是课件详情页把渲染包在 `v-if="auth.isTeacher"` 里，
注释写着「学生端不显示知识点，那是 AI 助手的回答素材，直接摊开就没必要问了」。
**该判断已被需求 6 推翻**，现已对两端开放。

新增的只是展示：课件详情页的知识点浏览、课堂内右侧的「知识点」标签页、
以及把课件+页码带进 AI 助手的「问 AI」入口。

#### 已实测通过（33 项）

会话 CRUD / 轮次 / **上下文记忆（AI 答得出上一轮问了什么）** /
**★ 不同会话问同一句得到不同答案（证明缓存确已移除）** / 课后提问 /
归属校验（他人会话 404）/ 幂等仍在 / 老接口不受影响。

---

### 6.14 新增需求 P4：管理端（M6，2026-09-21）

> 来源：`新增需求文档/M6_管理端.md`。
> **整棵 `/api/admin/**` 子树由一行配置拦死**，其余全是业务逻辑。

#### ⚠️ 唯一的安全边界（最重要的一节）

```java
// SecurityConfig，放在白名单之后、默认 deny 之前
.requestMatchers("/api/admin/**").hasRole("ADMIN")
```

漏了这一行**不是「功能不好用」**，而是任何登录学生都能删库、重置任何人密码。
**已实测**：学生与教师打全部管理端接口一律 **403**，未登录 **401**。

#### 账号管理 `/api/admin/users`

| 方法 | 路径 | 说明 |
|---|---|---|
| GET | `` | 列表。`keyword` / `role` / `disabled` 三个筛选都可选 |
| POST | `` | 新建。**可指定角色**（自助注册只能建学生） |
| GET | `/{id}` | 详情 |
| PUT | `/{id}/status` | `{disabled}` 禁用/启用 |
| PUT | `/{id}/password` | `{newPassword}` 重置密码 |
| PUT | `/{id}/role` | `{role}` 改角色 |
| DELETE | `/{id}` | 软删除 |

**禁用 ≠ 软删除**：禁用是临时的（账号还在、数据都在、列表里照样看得到），
软删除是移除（列表里不再出现）。所以 `user` 表新增了 `disabled` 列，
**不能拿 `deleted` 兼职**——那样一次「启用」会把删除也一并撤销。

**禁用 / 改角色 / 删除都会自增 `token_version`**，让该账号**已签发**的 JWT 立即失效。
不这么做的话，「禁用」只拦住了下一次登录，此刻已经在线的人照样能用到令牌过期（7 天）——
而管理员点「禁用」的预期是「他现在就用不了了」。

**三道护栏**（都会返回业务错误，不是 500）：
不能禁用自己、不能删除自己、不能把**最后一个**管理员降级或删除。
这三条都不是「技术上做不到」，而是「做到了就只能改数据库才救得回来」。

#### 课堂管理 `/api/admin/sessions`

| 方法 | 路径 | 说明 |
|---|---|---|
| GET | `` | 列表。`status` / `teacherId` / `keyword` 可选 |
| GET | `/{id}` / `/{id}/online` | 详情 / 实时在线名单 |
| POST | `/{id}/end` / `/{id}/pause` / `/{id}/resume` | 强制下课 / 暂停 / 恢复 |
| POST | `/{id}/kick` | `{userId, reason}` 移出学生 |
| POST | `/pause-all` `/resume-all` `/end-all` | 批量 |

> ⚠️ **批量接口必须声明在 `/{id}` 之前**，否则 `/sessions/pause-all` 会去匹配 `/{id}`，
> 把 "pause-all" 当 Long 解析失败，返回「参数格式不正确：id」。

**批量操作不做「一个失败全回滚」**，返回
`{total, succeeded, failed, failures:[{id,title,reason}]}`。
回滚的话管理员看到的是「点了没反应」，而且完全不知道哪几个生效了。

**踢人的顺序不能反**：先发 `kicked` 消息、**再**关连接。
先关的话那条提示发不出去，学生只会看到连接莫名断开然后自动重连回来——
管理员会以为踢人没生效，反复点。

**暂停的语义边界**（界面上必须写明）：媒体流是点对点直连的，**服务端管不到画面**。

| 会发生 | 不会发生 |
|---|---|
| 状态改为 `PAUSED` | ❌ 老师的画面不会被服务端切掉 |
| 广播 `class.paused`，学生端盖遮罩 | ❌ 声音不会停 |
| 讨论区与 AI 提问被禁用 | |
| 老师端收到提示，可自行停止共享 | |
...
| 方法 | 路径 | 说明 |
|---|---|---|
| GET | `/api/admin/coursewares` | 列表（含源文件与幻灯片占用） |
| DELETE | `/api/admin/coursewares/{id}` | 删除：**DB 行 + 源文件 + 幻灯片目录三处一起清** |
| POST | `/api/admin/coursewares/{id}/reparse` | 重新解析（**会真调付费模型**） |
| GET | `/api/admin/storage/overview` | 占用统计 |
| GET | `/api/admin/storage/orphans` | 孤立文件列表（**只列不删**） |
| POST | `/api/admin/storage/orphans/clean` | `{paths:[...]}` 清理 |

**为什么删除要封装成一个服务**：「删课件」很容易只做一半——删了库里的行，
磁盘上的 `.pptx` 与几十张幻灯片还留着。单份 25MB 起，删十份就是几百兆垃圾，
而且**没有任何界面能看出来**，只有等磁盘满。

**顺序是先提交事务、再删文件**。反过来一旦回滚，库里行还在、文件却没了，
那个课件就永远打不开。现在的顺序最坏是「行已删、文件残留」——正好是孤立文件清理能兜住的。

**孤立文件只列不删**：`clean` 必须**显式带上要删的路径**（没有「一键清空」的形式），
而且服务端**会重新校验一次**——从「列出」到「确认删除」之间磁盘可能已经变了。
路径还会被校验是否落在存储根目录之内（已实测 `../../application-local.yml` 被拒、实删 0）。

#### 审计日志 `/api/admin/audit-logs`

只做「写 + 查」，**不配图表与统计**（按需求）。写入统一走
`AdminAuditService.record(adminId, action, targetType, targetId, detail)` 收口一处。

- 用 **`REQUIRES_NEW`** 独立事务：业务回滚不该把日志一起回滚掉——
  「管理操作失败了」恰恰是更该留下记录的事。
- 写入失败**只记日志、不抛异常**：审计是事后追责用的，不是业务正确性的前提。
- 表里只存 `adminId`（**刻意不建外键关联**），操作人名字在查询时批量补上。

**动作码**：`USER_CREATE` / `USER_DISABLE` / `USER_ENABLE` / `USER_RESET_PWD` /
`USER_CHANGE_ROLE` / `USER_DELETE` / `SESSION_FORCE_END` / `SESSION_PAUSE` /
`SESSION_RESUME` / `SESSION_PAUSE_ALL` / `SESSION_RESUME_ALL` / `SESSION_END_ALL` /
`SESSION_KICK` / `COURSEWARE_DELETE` / `COURSEWARE_REPARSE` / `STORAGE_ORPHAN_CLEAN`

#### 已实测通过（48 项）

学生与教师打全部管理端接口 403 / 未登录 401 / 用户 CRUD 全通 /
禁用后立刻登不进 / 三道护栏生效 / 批量汇总正确 / 路径穿越被拒 /
审计 11 种动作全部留痕且带操作人名。

#### ⚠️ 又一处计划书缺漏（已补）

`user` 表原本**没有 `disabled` 列**——M6 的接口列表里有「禁用/启用账号」，
但数据库里只有 `deleted`（软删除）。两者不能混用（见上），已通过迁移补上。
